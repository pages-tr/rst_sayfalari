...
or */etc/csh.cshrc*. (See also **dircolors**\ (1).) Usually, the file
used here is */etc/DIR_COLORS* and can be overridden by a *.dir_colors*
...
file is considered valid for all terminal types. Following the global
section is one or more *terminal-specific* sections, preceded by one or
more **TERM** statements which specify the terminal types (as given by
the **TERM** environment variable) the following declarations apply to.
...
**TERM terminal-type**
   ...
   applies to. Multiple **TERM** statements can be used to create a
   section which applies for several terminal types.

**COLOR yes|all|no|none|tty**
   (Slackware only; ignored by GNU **dircolors**\ (1).) Specifies that
   colorization should always be enabled (*yes* or *all*), never enabled
   (*no* or *none*), or enabled only if the output is a terminal
...
**ORPHAN color-sequence**
   ...
   points to a nonexistent file). If this is unspecified, **ls** will
...
**.**\ *extension* *color-sequence*
   ...
   starting with a period, such as **~** for **emacs** backup files.
...
6429 color codes and harmlessly eliminate them from the output or
emulate them. **ls** uses ISO 6429 codes by default, assuming
...
text gets colorized after you do a directory listing, change the
...
sequence: **LEFTCODE** *typecode* **RIGHTCODE** *filename* **ENDCODE**,
where the *typecode* is the color sequence that depends on the type or
name of file. If the **ENDCODE** is undefined, the sequence **LEFTCODE
NORMAL RIGHTCODE** will be used instead. The purpose of the left- and
rightcodes is merely to reduce the amount of typing necessary (and to
hide ugly escape codes away from the user). If they are not appropriate
...
setup file, it *cannot* be undefined in a terminal-specific section of
the file. This means any **NORMAL** definition will have no effect. A
...
**stty**-style ^-notation can be used. The C-style notation includes the
...
The default **LEFTCODE** and **RIGHTCODE** definitions, which are used
by ISO 6429 terminals are:
