...
password authentication. If you are planning on using the **crypt**\ ()
...
should do with this structure is to set *data->initialized* to zero
...
The **crypt**\ (), **encrypt**\ (3), and **setkey**\ (3) functions are
part of the POSIX.1-2008 XSI Options Group for Encryption and are
optional. If the interfaces are not available, then the symbolic
constant **\_XOPEN_CRYPT** is either not defined, or it is defined to -1
and availability can be checked at run time with **sysconf**\ (3). This
may be the case if the downstream distribution has switched from glibc
crypt to *libxcrypt*. When recompiling applications in such
distributions, the programmer must detect if **\_XOPEN_CRYPT** is not
available and include *<crypt.h>* for the function prototypes; otherwise
*libxcrypt* is an ABI-compatible drop-in replacement.
...
"$\ *id*\ $" followed by a string optionally terminated by "$", then the
result has the form:
...
contain the password encrypted with, respectively, functions based on
SHA-256 and SHA-512.
...
"$\ *id*\ $" characters in the salt are followed by "rounds=\ *xxx*\ $",
...
rounds actually used is 1000 if *xxx* is less than 1000, 999999999 if
*xxx* is greater than 999999999, and is equal to *xxx* otherwise.

SEE ALSO
========

**login**\ (1), **passwd**\ (1), **encrypt**\ (3), **getpass**\ (3),
**passwd**\ (5)
